...
grow1 = mass~bill_l+bill_d+flip_l+species+sex+island
grow2 = mass~bill_l+bill_d+flip_l+species+sex
grow3 = mass~bill_d+flip_l+species+sex

m1 = lm(grow1,data=penguins_clean)
m2 = lm(grow2,data=penguins_clean)
m3 = lm(grow3,data=penguins_clean)
```
...
- Set testing data to the side
-- use random sampling
- Determine the # of training data we want to use (K=10)
- For each model
-- Apply model to each training set
-- Make prediction on test data with fitted model

- Set metric for accuracy (Root mean squared error: RMSE)
- Summarize RMSE to determine which model was best on avg
- Final model built using entire dataset
...
training_lm1 = lm(grow1,data=train_df)
training_lm2 = lm(grow2,data=train_df)
training_lm3 = lm(grow3,data=train_df)
...
# create a vector of our calculations
...
set.seed(42)
  ...
  training_df = penguins_fold |>
    filter(group!=i)
  testing_df = penguins_fold |>
    filter(group==i)
  
  # Run for one model
  model_lm = lm(grow1, data=training_df)
  
  #Get the predictions from the model
  predict_test = testing_df |>
    mutate(model=predict(model_lm,testing_df))
  
  # Summarize/calculate the rmse for that model
  kfold_rmse = predict_test |>
    summarize(rmse=calc_rmse(mass,model))
...
model_final = lm(grow1,data=penguins_df)
```
...
kfold_cv<-function(i,df,formula){
  ...
  # Run for one model
  model_lm = lm(formula, data=training_df)
...
rmse_df<-data.frame(j=1:folds) |> 
  mutate(rmse_mdl1=map_dbl(j, kfold_cv, df=penguins_fold,formula=grow1),
         rmse_mdl2=map_dbl(j,kfold_cv,df=penguins_fold,formula=grow2),
         rmse_mdl3=map_dbl(j,kfold_cv,df=penguins_fold,formula=grow3))
